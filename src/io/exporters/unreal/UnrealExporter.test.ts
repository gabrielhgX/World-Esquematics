...
const makeWorld = (extent_m = 2048, resolution_m = 4) =>
  new WorldData(
    createWorldConfig({
      projectName: 'Teste',
...
describe('nearestLandscapeSize (README §9.1, gotcha #1)', () => {
  it('escolhe o tamanho válido mais próximo', () => {
    expect(nearestLandscapeSize(4000)).toBe(4033);
    expect(nearestLandscapeSize(505)).toBe(505);
    expect(nearestLandscapeSize(512)).toBe(505);
    expect(nearestLandscapeSize(200)).toBe(253);
    expect(nearestLandscapeSize(50)).toBe(127);
    expect(nearestLandscapeSize(20000)).toBe(8129);
  });
});
...
describe('UnrealExporter — heightmap mínimo (README §9.1)', () => {
  it('validate avisa sobre a reamostragem quando o grid não bate com a tabela', () => {
    const world = makeWorld(); // grid 512² → 505²
    const issues = exporter.validate(world);
    expect(issues.some((i) => i.severity === 'warning' && i.message.includes('505'))).toBe(true);
    expect(issues.every((i) => i.severity !== 'error')).toBe(true);
  });

  it('exporta o .r16 na resolução válida + manifesto de import', async () => {
    ...
    expect(bundle.notes.some((n) => n.includes('reamostrado'))).toBe(true);
  });
  ...
  it('gotcha #2: ZScale e LocationZ corretos para o heightRange', async () => {
    ...
    // Escala XY: 2048 m / 504 quads ≈ 4.0635 m → ×100 UE units
    expect(manifest.landscape.scale.x).toBeCloseTo((2048 / 504) * 100, 6);
  });
  ...
  it('gotcha #3: mapa "L" assimétrico sai sem espelhamento (README §9.1/§11)', async () => {
    ...
    const world = makeWorld(); // grid 512², exporta 505²
